...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	createUsersListItemQuery := fmt.Sprintf("INSERT INTO %s (list_id, item_id) VALUES ($1, $2)", ListsItemsTable)
	_, err = tx.Exec(createUsersListItemQuery, listId, itemId)
	if err!= nil {
		tx.Rollback()
        return 0, err
    }
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul 
							WHERE  ti.id = li.item_id AND li.list_id = ul.list_id 
							AND ul.user_id = $%d AND ti.id = $%d`, todoItemsTable, setQuery, ListsItemsTable, usersListTable, argId, argId+1)
